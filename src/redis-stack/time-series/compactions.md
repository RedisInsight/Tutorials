Redis time series allow you to compact your time series data and make it available in various sized aggregations.

The following commands will create containers for compacted data and rules for how that data should be aggregated.

```redis Time series compaction
TS.CREATE bike_sales_1_per_day LABELS region east compacted yes // a new time series for compacted bike_sales_1 data
TS.CREATERULE
...
Data in the original time series keys, bike_sales_1, etc., is left intact.
...
When rules are deleted, the related time series are not deleted.

```redis Re-create the deleted rule
